class AppliesController < ApplicationController
    ...
    def create 
        if login?
            # check if this is the group created by user self
            @group = Group.find(params[:group_id])
            if @current_user.id == @group.founder_id
                render json: {founder_error: 'This is group created by yourself.'}, status: 400
            else
